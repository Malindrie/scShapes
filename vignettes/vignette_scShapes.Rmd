...
```{r message=FALSE, warning=FALSE}
scData_models <- fit_models(counts=scData.sig.genes, cexpr=scData$covariates, lib.size=scData$lib_size, BPPARAM=bpparam())
```
...
```{r message=FALSE, warning=FALSE}
scData_gof <- gof_model(scData_least.bic, cexpr=scData$covariates, lib.size=scData$lib_size, BPPARAM=bpparam())
```
